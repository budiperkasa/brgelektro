<?php echo $header; ?>
	...
	<?php 	
		if ($column_left || $column_right) { 
			$main = "col-sm-9"; 
			if($column_left) $row=" sideleft"; else $row= " sideright";
		} else { $main = "col-sm-12"; $row = '-fluid'; }
	?>
							...
							for ($i = 1; $i <= $this->config->get('slides_quantity'); $i++) { 
								...
								if(isset($slider[$i][$language_id]['status'])) {
									...
									if($slider[$i][$language_id]['status'] == '1') {
										...
										for ($m = 1; $m <= $this->config->get('elements_quantity'); $m++) {
											...
											if($slider[$i][$language_id][$m]['status'] == '1') {
												...
												$extraclass = false;
													...
													$element = '<iframe width="100%" height="100%" src="http://www.youtube.com/embed/'. $videoID .'?html5=1&amp;rel=0" frameborder="0" allowfullscreen></iframe>';
										...
										echo '<li data-transition="'.$slider[$i][$language_id]['transition_effect'].'" data-slotamount="10" data-masterspeed="300"'.$link.'><img src="image/'.$image.'" '.$bgslide.' data-fullwidthcentering="on">'.$description.'</li>';
					...
					<script type="text/javascript">
						...
						if ($.fn.cssOriginal!=undefined)
							...
							$('.fullwidthbanner').revolution({
								...
								<?php $layout = $this->config->get('fortuna_main_layout'); ?>

								startwidth:<?php echo ($layout=='large') ? '1200' : '960';  ?>,
